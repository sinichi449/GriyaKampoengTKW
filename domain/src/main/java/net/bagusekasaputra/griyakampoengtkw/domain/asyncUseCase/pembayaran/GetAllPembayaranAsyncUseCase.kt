...
class GetAllPembayaranAsyncUseCase(
    ...
    override fun process(request: Request): Flow<Result<List<Pembayaran>?>> {
        return pembayaranRepository.getAllPembayaran(request.kavlingKode, request.dataMode)
            .zip(getHargaKavling(request.kavlingKode, request.dataMode)) { resultListPembayaran, hargaKavling ->
    ...
    private fun getHargaKavling(kavlingKode: String, dataMode: DataMode): Flow<HargaKavling> {
        return hargaKavlingRepository.getHargaKavling(kavlingKode, dataMode).map {
            it.getOrNull()
